...
const Jobs = () => {
    ...
    useEffect(() => {
        ...
        // Основные фильтры
        if (filter["Должность"]?.length > 0) filtered = filtered.filter((job: any) =>
            filter["Должность"].some((x: any) => job.jobTitle?.toLowerCase().includes(x.toLowerCase())));
        ...
        if (filter["Формат работы"]?.length > 0) {
            filtered = filtered.filter((job: any) => {
                return filter["Формат работы"].some((x: any) => {
                    if (x === "В университете") {
                        return job.company === user.university;
                    }
                    return job.workFormat && job.workFormat.toLowerCase().includes(x.toLowerCase());
                });
            });
        }
